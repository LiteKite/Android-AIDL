...
class BankService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        Log.d(TAG, "onCreate")
        bankDatabase = BankDatabase.getBankDatabase(this)
    }
    ...
    inner class BankBinder : IBankService.Stub() {
        ...
        override fun signupRequest(signupRequest: SignupRequest?) {
            ...
            coroutineScope.launch {
                val isUserExists = bankDatabase.isUserExists(signupRequest.username)
                if (isUserExists) {
                    ...
                    val userId = bankDatabase.saveUserAccount(userAccount)
        ...
        override fun loginRequest(loginRequest: LoginRequest?) {
            ...
            coroutineScope.launch {
                val isUserExists = bankDatabase.isUserExists(loginRequest.username)
                if (!isUserExists) {
                    postFailureResponse(
                        RequestCode.LOGIN,
        ...
        override fun userDetailsRequest(userId: Long) {
            coroutineScope.launch {
                val userAccount = bankDatabase.getUserAccount(userId)
                if (userAccount != null) {
                    remoteBroadcast { index ->
        ...
        override fun depositRequest(userId: Long, amount: Double) {
            coroutineScope.launch {
                ...
                if (userAccount != null) {
                    ...
                    val noOfRowsUpdated = bankDatabase.updateUserAccount(userAccount)
        ...
        private fun postCurrentBalanceChanged(currentBalance: Double) {
            remoteBroadcast { index ->
                bankServiceCallbacks.getBroadcastItem(index).onCurrentBalanceChanged(currentBalance)
            }
        }
